...
export default function () {
    ...
    const addExpenses = (e) => {
        ...
        let divideAmount = (parseInt(parseInt(formData.get("amount")) / name.length));
        let c = 0;
        name.map((item) => {
            if (item.name == formData.get("paidvalue")) {
                let a = (parseInt(parseInt(formData.get("amount")) / name.length)) - parseInt(formData.get("amount"));
                item.amount = item.amount + a;
            }
            else {
                if (item.amount < 0)
                    c = c + 1;
                let a = (parseInt(parseInt(formData.get("amount")) / name.length));
                item.amount = item.amount + a;
            }
        });
        // name.map((item) => {
    ...
    <h1 className="justify-center text-center text-red-700 text-xl">Note: Before adding Expenses, add options on the left side and then add expenses.</h1>
        <Container className="flex items-center justify-center gap-8">
             <section class="bg-white dark:bg-gray-900">
                <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
                    <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Add a new option for PaidBy field</h2>
                    <form action="#" onSubmit={addNamesToLocalStorage}>
                        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                            <div class="sm:col-span-2">
                                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Expense Description</label>
            ...
            <div>
                <button className="bg-yellow-600 w-56 h-40 shadow-lg shadow-yellow-500/50 text-6xl"><h3 className="text-xl">Total Expenses</h3>{total}</button>
            </div>
            <section class="bg-white dark:bg-gray-900">
                <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
        ...
        <Container className="flex flex-wrap  items-center justify-center">
            {name.map((value) => {
                return (
                    <div className="p-4">
                        <button className="bg-gray-400 w-56 h-40 shadow-lg shadow-gray-400/50 text-6xl"><h3 className="text-xl uppercase">{value.name}</h3>{value.amount}</button>
                    </div>
                )
            }
            )}
